...
PRIVATE
PUBLIC :: sagmgpar_relmem, sagmgpar_partroword
PUBLIC :: sagmgpar_setupL1, sagmgpar_smoothsetup
...
CONTAINS
  ...
  SUBROUTINE sagmgpar_partroword(n, a, ja, ia, idiag, w, iw, iext, nzext)
    IMPLICIT NONE
    INTEGER :: n, ja(*), ia(n+1), idiag(n)
    INTEGER, OPTIONAL :: iext(*), nzext
    INTEGER, TARGET :: iw(*)
    REAL(kind(0.0e0)) :: a(*), p
    REAL(kind(0.0e0)), TARGET :: w(*)
  ...
  SUBROUTINE sagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    REAL(kind(0.0e0)) :: ALPHA, BET0, RHO, dum(3), td
    REAL(kind(0.0e0)) :: BET1
    REAL(kind(0.0e0)), ALLOCATABLE :: SD(:)
    REAL(kind(0.0e0)), ALLOCATABLE :: S(:),fsc(:)
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ...
       END IF
       IF ( ITER.GT.1 ) THEN
          dum(1) = - SDOT(N, SD, IONE, S(1+3*N), IONE)
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_REAL,  &
               MPI_SUM,ICOMM,ierr)
          BET0=dum(1)
          BET1=BET0/RHO
          CALL SAXPY(N, BET1, S(1+2*N), IONE, S(1+3*N), IONE)
          flop=flop+real(4*N)
          IF (intmv) THEN
             CALL SAXPY(N, BET1, SD, IONE, S(1+4*N), IONE)
             flop=flop+real(2*N)
          END IF
       ENDIF
       CALL SCOPY(N, S(1+3*N), IONE, S(1+2*N), IONE)
       IF (intmv)  THEN
          CALL SCOPY(N, S(1+4*N), IONE, SD, IONE)
       ELSE
          IF (intmv2) THEN
          ...
          END IF
       END IF
       dum(1) =  SDOT(N, S(1+2*N), IONE, SD, IONE)
       dum(2) =  SDOT(N,S(1+2*N),IONE,f,IONE)
       flop=flop+real(4*N)
       IF (ITER.EQ.1) THEN
          ...
          END IF
       ELSE
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:2),2,MPI_REAL,  &
               MPI_SUM,ICOMM,ierr)
       END IF
       RHO=dum(1)
       ALPHA=dum(2)/RHO
       IF (ITER.EQ.1 .AND. (.NOT.init)) THEN
          CALL SCOPY(N,S(1+2*N),IONE,X,IONE)
          CALL SSCAL(N,ALPHA,X,IONE)
          flop=flop+real(N)
       ELSE
          CALL SAXPY(N, ALPHA, S(1+2*N), IONE, X, IONE)
          flop=flop+real(2*N)
       END IF
       CALL SAXPY(N, -ALPHA, SD, IONE, f, IONE)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE sagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    REAL(kind(0.0e0)), ALLOCATABLE :: Su(:),R(:),fsc(:),Sw(:)
    LOGICAL :: init,intmv,intmv2
    REAL(kind(0.0e0)), external :: SDOT
    REAL(kind(0.0e0)), external :: SNRM2
    INTEGER , parameter :: IONE=1
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID2.GT.TOL2BNORM2 )
       ...
       IF (itm.GT.0) THEN
          DO i=0,itm-1
             j=MOD(i,2)+4
             dum(1)=SDOT(N,Sc(1+i*N),IONE,Sc(1+itm*N),IONE)
             CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_REAL,  &
                  MPI_SUM,ICOMM,ierr)
             bet0=dum(1)
             bet0=bet0/SiR(1+i+(i*(i+1))/2)
             SiR(1+i+(itm*(itm+1))/2)=bet0
             CALL SAXPY(N,-bet0,Sc(1+i*N),IONE,Sc(1+itm*N),IONE)
             flop=flop+real(4*N)
          END DO
       END IF
       IF (j.EQ.4) THEN
       ...
       ELSE
          j=4
       END IF
       dum(1)=SNRM2(N,Sc(1+itm*N),IONE)**2
       dum(2)=SDOT(N, Sc(1+itm*N), IONE, f, IONE)
       flop=flop+real(6*N)
       IF (ITER.EQ.1) THEN
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_REAL,  &
               MPI_SUM,ICOMM,ierr)
       ...
       bet0=alpha/rho
       Siv(1+itm)=bet0
       CALL SAXPY(N, -bet0, Sc(1+itm*N), IONE, f, IONE)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  RECURSIVE SUBROUTINE sagmgpar_inner_iter(n,X,R,l)
    ...
    dum(1) = SDOT(N,X,IONE,R(1,2),IONE)
    dum(2) = SDOT(N,X,IONE,R,IONE)
    dum(3) = SNRM2(N, R, IONE)**2
    dum(4) = SNRM2(N,R(1,2),IONE)**2
    dum(5) = SDOT(N, R, IONE, R(1,2), IONE )
    ...
    IF (RESID.LE.resi*resi*BNORM) THEN
       ...
       RETURN
    END IF
    CALL SAXPY(N, -bet0, R(1,2), IONE, R, IONE)
    ITER = 2
    CALL sagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = SDOT(N,R(1,3),IONE,R(1,2),IONE)
    dum(2) = SDOT(N,R(1,3),IONE,R,IONE)
    dum(3) = SDOT(N,R(1,3),IONE,R(1,4),IONE)
    IF (.NOT.spd) dum(4) = SDOT(N,X,IONE,R(1,4),IONE)
    ...
    CALL SAXPY(N, zeta, R(1,3), IONE, X, IONE)
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+real(21*N)
    IF (.NOT.spd) flop=flop+real(2*N)
    RETURN
100 CONTINUE
    dum(1)=SNRM2(N,R(1,2),IONE)**2
    dum(2)=SDOT(N, R(1,2), IONE, R, IONE )
    dum(3) = SNRM2(N, R, IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_REAL,  &
            MPI_SUM,ICOMM,ierr)
    IF (dum(3).EQ.0.0e0) THEN
...
110 CONTINUE
    ...
    ITER = 2
    CALL sagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = SDOT(N,R(1,4),IONE,R(1,2),IONE)
    dum(2) = SDOT(N,R(1,4),IONE,R,IONE)
    dum(3) = SNRM2(N,R(1,4),IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_REAL,  &
            MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
...
200 CONTINUE
    dum(1) = SNRM2(N,R(1,2),IONE)**2
    dum(2) = SDOT(N,R(1,4),IONE,R(1,2),IONE)
    dum(3) = SNRM2(N,R(1,4),IONE)**2
    dum(4) = SDOT(N,R(1,2),IONE,R,IONE)
    dum(5) = SDOT(N,R(1,4),IONE,R,IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_REAL,  &
            MPI_SUM,ICOMM,ierr)
     ...
     CALL SAXPY( N, y2, R(1,3), IONE, X, IONE )
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+real(25*N)
    RETURN
...
200 CONTINUE
  ...
  SUBROUTINE sagmgpar_setupL1(n,a,ja,ia,listrank,ifl)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    nglp=ngl
    IF (1 /= nlev) THEN
       CALL sagmgpar_aggregation(1,n,a,ja,ia,nc,listrank)
       CALL sagmgpar_setup(2,nc,listrank)
    ELSE
       DEALLOCATE(dt(1)%idiag)
       CALL sagmgpar_MUMPSpar(n,fff,1,a,ja,ia)
       nwrkcum=1
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(real(ngl(1))/maxcoarset)
      CALL sagmgpar_restorelstrank(ilstinL1,listrank,ifl)
      ifl=n+1
       RETURN
    END IF
       CALL sagmgpar_restorelstrank(ilstinL1,listrank,ifl)
...
958 FORMAT('****','              Weighted complexity:',f9.2)
  ...
  RECURSIVE SUBROUTINE sagmgpar_setup(l,n,listrank)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n
    ...
    END IF
    IF (l /= nlev) THEN
       CALL sagmgpar_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc,listrank)
       CALL sagmgpar_setup(l+1,nc,listrank)
    ELSE
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(real(ngl(1))/maxcoarset)
...
935 FORMAT(i3,'*','       Number of local rows:',i12)
  ...
  SUBROUTINE sagmgpar_aggregation(l,n,a,ja,ia,nc,listrank)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n,nc
    INTEGER :: ja(*),ia(n+1)
...
999 CONTINUE
    ...
    isize=MAX(isize,1)
    ALLOCATE (jan(isize))
    CALL sagmgpar_setextnum(nc,l,jan,nzp,listrank)
    IF (nc.EQ.0) THEN
        DEALLOCATE(jan)
        RETURN
    ENDIF
...
901    FORMAT(i3, &
  ...
  SUBROUTINE sagmgpar_setsendrecL1(n,ja,ia,iext,listrank,ifl,nzext,news)
    USE sagmgpar_mem
    IMPLICIT NONE
...
1010 FORMAT(' The new input matrix has on rank',i9,'has external'       &
  ...
  SUBROUTINE sagmgpar_setextnum(nc,l,iw,imax,listrank)
    USE sagmgpar_mem
    IMPLICIT NONE
...
!!#ifdef
  ...
  SUBROUTINE sagmgpar_setcgext(l,n,a,ja,ia,idiag,iext,listrank,ifl   &
                            ,ao,jao,iw,nout)
    USE sagmgpar_mem
    IMPLICIT NONE
  ...
  SUBROUTINE sagmgpar_LAPACK(n,f,ijb,a,ja,ia)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n,ijb
    REAL(kind(0.0e0)), TARGET :: f(n)
    ...
    INTEGER, OPTIONAL :: ia(n+1)
    INTEGER, OPTIONAL, TARGET :: ja(*)
    REAL(kind(0.0e0)), ALLOCATABLE, SAVE :: ac(:,:)
    ...
    ELSE IF (ijb.EQ.1) THEN
       ...
       DO i=1,n
          ...
          END DO
       END DO
       CALL SGETRF(N,N,ac,N,ipiv,ierr)
       IF (ierr /= 0) THEN
    ...
    ELSE IF (ijb.EQ.2) THEN
       CALL SGETRS('N',N,IONE,ac,N,ipiv,f,N,ierr)
       IF (ierr /= 0) THEN
    ...
    END IF
    RETURN
  END SUBROUTINE sagmgpar_LAPACK
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    REAL(kind(0.0e0)), POINTER, DIMENSION(:) :: ascal
    REAL(kind(0.0e0)), POINTER, DIMENSION(:) :: w
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    IF (MOD(ijb,10).GE.2) THEN
      IF (ijob.LE.201) THEN
        ...
        CALL sagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1,nzext)
        CALL sagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank,nzext &
                                ,.FALSE.)
    ...
    ELSE
     ...
     END IF
      ...
      CALL sagmgpar_setupL1(n,a,ja,ia,listrank,ifirstlistrank)
