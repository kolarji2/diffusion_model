...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This file provides dagmgpar and dependencies; dagmgpar is a
! parallel (MPI) implementation for real matrices in double precision
...
PRIVATE
PUBLIC :: dagmgpar_relmem, dagmgpar_partroword
PUBLIC :: dagmgpar_setupL1, dagmgpar_smoothsetup
PUBLIC :: dagmgpar_applyprec, dagmgpar_GCR, dagmgpar_FlexCG
PUBLIC :: dagmgpar_setsendrecL1
PUBLIC :: dagmgpar_restorelstrank
CONTAINS
  SUBROUTINE dagmgpar_relmem(ijb)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l, ier, ijb
    REAL(kind(0.0d0)) :: fdum(1)
    DO l=1,nlev-1
       IF(nn(l).GT.0) THEN
            IF (nn(l+1).GT.0)     &
                      DEALLOCATE(dt(l)%ind)
  ...
  SUBROUTINE dagmgpar_applyprec( N,f,X,ijb,a,ja,ia)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: N, ijb
    ...
    IF (iter.GT.1) THEN
       CALL dagmgpar_prec_matv(1,f,X,S(N+1),S(2*N+1),.TRUE.)
       X(1:N)=tau*X(1:N)
       DO i=2,iter-1
          CALL dagmgpar_prec_matv(1,f,S,S(N+1),S(2*N+1),.TRUE.)
          X(1:N)=X(1:N)+tau*S(1:N)
          f(1:N)=f(1:N)-tau*S(N+1:2*N)
       END DO
       CALL dagmgpar_prec_matv(1,f,S,S(N+1),S(2*N+1),.FALSE.)
       X(1:N)=X(1:N)+tau*S(1:N)
    ELSE
       CALL dagmgpar_prec_matv(1,f,X,S,S(N+1),.FALSE.)
    END IF
    kstat(2,1)=kstat(2,1)+iter
    DEALLOCATE(S)
    RETURN
  END SUBROUTINE dagmgpar_applyprec
  SUBROUTINE dagmgpar_partroword(n, a, ja, ia, idiag, w, iw, iext, nzext)
    IMPLICIT NONE
  ...
  SUBROUTINE dagmgpar_sendrec(vect,ivect,ishift,l)
    USE dagmgpar_mem
    IMPLICIT NONE
  ...
  SUBROUTINE dagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    ...
    REAL(kind(0.0d0)) :: ALPHA, BET0, RHO, dum(3), td
    ...
    IF (init) THEN
       IF (intmv) THEN
          CALL dagmgpar_sendrec(x,idum,-1,1)
          IF (smoothtp.EQ.1) THEN
             f(1:n)=f(1:n)-x(1:n)/dt(1)%a(1:n)
          ELSE
             f(1:n)=f(1:n)-x(1:n)*(dt(1)%a(1:n)-1/(dt(1)%p(1:n)))
          END IF
          CALL dagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
          CALL dagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iu,x,1,f,-2,flop)
          IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ierr)
          CALL  dagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iext,buffi,n+1,f &
               ,-2,flop,dt(1)%lstin)
       ELSE
          IF (intmv2) THEN
             CALL dagmgpar_matv(N,x,SD,dt(1)%a,dt(1)%ja,dt(1)%ia,transint &
       ...
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(2:3),2,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
       BNORM=SQRT(dum(3))
       RESID=SQRT(dum(2))
       RESID0=RESID
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ITER = ITER + 1
       IF (ijb.GT.201) THEN
          CALL dagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv)
       ELSE
          CALL dagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
       END IF
       IF ( ITER.GT.1 ) THEN
          dum(1) = - DDOT(N, SD, IONE, S(1+3*N), IONE)
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_DOUBLE_PRECISION,  &
               MPI_SUM,ICOMM,ierr)
          BET0=dum(1)
          BET1=BET0/RHO
          ...
          flop=flop+dble(4*N)
          IF (intmv) THEN
             CALL DAXPY(N, BET1, SD, IONE, S(1+4*N), IONE)
             flop=flop+dble(2*N)
          END IF
       ENDIF
       CALL DCOPY(N, S(1+3*N), IONE, S(1+2*N), IONE)
       IF (intmv)  THEN
          CALL DCOPY(N, S(1+4*N), IONE, SD, IONE)
       ELSE
          IF (intmv2) THEN
             CALL dagmgpar_matv(N,S(1+2*N),SD,dt(1)%a,dt(1)%ja,dt(1)%ia,trans &
             ,dt(1)%iext,dt(1)%lstin,1)
          ELSE
       ...
       flop=flop+dble(4*N)
       IF (ITER.EQ.1) THEN
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_PRECISION,  &
               MPI_SUM,ICOMM,ierr)
          IF (.NOT.init) THEN
             BNORM=SQRT(dum(3))
             RESID0=BNORM
          ...
          END IF
       ELSE
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:2),2,MPI_DOUBLE_PRECISION,  &
               MPI_SUM,ICOMM,ierr)
       END IF
       RHO=dum(1)
       ALPHA=dum(2)/RHO
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE dagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    REAL(kind(0.0d0)) ::  f(N), X(N)
    ...
    IF (init) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL dagmgpar_matv(N,x,Sc,dt(1)%a,dt(1)%ja,dt(1)%ia,trans &
             ,dt(1)%iext,dt(1)%lstin,1)
          ELSE
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID2.GT.TOL2BNORM2 )
       ...
       END IF
       IF (ijb.GT.201) THEN
          CALL dagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv)
       ELSE
          CALL dagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
       END IF
       IF (.NOT.intmv) THEN
          IF (intmv2) THEN
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE dagmgpar_matv(n, x, y, a, ja, ia, transpose,         &
    ...
    REAL(kind(0.0d0)) :: x(n)
    LOGICAL :: transpose
    CALL dagmgpar_sendrec(x,idum,-1,l)
  ...
  RECURSIVE SUBROUTINE dagmgpar_prec_matv(l,B,X,AX,R,matv,a,ja,ia)
    ...
    IF (nnext.GT.0) THEN
       ...
       END IF
       IF (l+1.EQ.nlev) THEN
          CALL dagmgpar_MUMPSpar(nnext,R(BN),2)
       ELSE IF ( innermax(l+1).LE.1 ) THEN
          AXN=BN+nnext
          RN=AXN+nnext
    ...
    IF (update) THEN
       ...
       ELSE
         CALL dagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%p   &
                              ,R,AX,1,flop)
       END IF
       CALL dagmgpar_sendrec(R,idum, -1,l)
       X(1:n)=X(1:n)+R(1:n)
       flop=flop+dble(n)
       IF (smoothtp.EQ.1) THEN
  ...
  SUBROUTINE dagmgpar_fwbwsolve1(l,B,X,AX,update,R)
    ...
    ELSE
       ...
       CALL dagmgpar_sendrec(R(n+1),idum,-1,l)
       AX(1:n)=AX(1:n)-R(1:n)
       flop=flop+dble(2*n)
       IF (smoothtp.EQ.1) THEN
  ...
  RECURSIVE SUBROUTINE dagmgpar_inner_iter(n,X,R,l)
    ...
    dum(5) = DDOT(N, R, IONE, R(1,2), IONE )
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
    ...
    IF (.NOT.spd) dum(4) = DDOT(N,X,IONE,R(1,4),IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:4),4,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
    rho2  =dum(3)
...
200 CONTINUE
  ...
  SUBROUTINE dagmgpar_setupL1(n,a,ja,ia,listrank,ifl)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    ELSE
      ...
      CALL dagmgpar_restorelstrank(ilstinL1,listrank,ifl)
      ifl=n+1
       RETURN
    END IF
       CALL dagmgpar_restorelstrank(ilstinL1,listrank,ifl)
...
999 CONTINUE
    ...
    isize=MAX(isize,1)
    ALLOCATE (jan(isize))
    CALL dagmgpar_setextnum(nc,l,jan,nzp,listrank)
    IF (nc.EQ.0) THEN
        DEALLOCATE(jan)
        RETURN
    ENDIF
...
901    FORMAT(i3, &
  ...
  SUBROUTINE dagmgpar_setsendrecL1(n,ja,ia,iext,listrank,ifl,nzext,news)
    USE dagmgpar_mem
    IMPLICIT NONE
...
1010 FORMAT(' The new input matrix has on rank',i9,'has external'       &
  ...
  SUBROUTINE dagmgpar_setextnum(nc,l,iw,imax,listrank)
    USE dagmgpar_mem
    IMPLICIT NONE
...
!!#ifdef
  ...
  SUBROUTINE dagmgpar_restorelstrank(ilstin,listrank,ifl)
    USE dagmgpar_mem
    IMPLICIT NONE
  ...
  SUBROUTINE dagmgpar_globnum(n,ja,ia,iext,irg,jcg,l,lstin,nloc)
    USE dagmgpar_mem
    IMPLICIT NONE
    ...
    DO i=1,n
      irg(i)=i
    END DO
    CALL dagmgpar_sendrec(dum,irg,ishift,l)
    DO i=1,n
       DO k=ia(i),iext(i)-1
          irg(k)=i+ishift
          jcg(k)=ja(k)+ishift
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ijb=MOD(ijob,100)
    itef=iter
    CALL dagmgpar_init(MPI_COMM_AGMG)
    IF (ijob.NE.-1 .AND. &
        ijob.NE. 0 .AND. &
        ijob.NE. 1 .AND. &
        ijob.NE. 2 .AND. &
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    IF (MOD(ijb,10).GE.2) THEN
      IF (ijob.LE.201) THEN
        ...
        CALL dagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1,nzext)
        CALL dagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank,nzext &
                                ,.FALSE.)
        CALL dagmgpar_restorelstrank(ilstinL1,listrank,ifirstlistrank)
        DEALLOCATE(dt(1)%idiag,w,iw)
      END IF
    ELSE
     IF (preprocessed) THEN
       CALL dagmgpar_relmem(-1)
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
       flop=0.0d0
   ...
   !              WRITE(iout,996) IRANK,cputmp
    ...
    IF (MOD(ijb,10).GE.3) THEN
       ...
       END IF
       CALL dagmgpar_mestime(-2,0.0d0,0.0d0)
       CALL dagmgpar_applyprec(n,f,x,MOD(ijb,10))
    ...
    ELSE IF (nrst.GT.1) THEN
       CALL dagmgpar_GCR(n,f,x,itef,resid,a,ja,ia,ijob)
    ELSE
       CALL dagmgpar_FlexCG(n,f,x,itef,resid,a,ja,ia,ijob)
    END IF
    iter=itef
