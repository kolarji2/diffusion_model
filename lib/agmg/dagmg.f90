...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This file provides dagmg and dependencies; dagmg is a
! sequential implementation for real matrices in double precision
...
PRIVATE
PUBLIC :: dagmg_relmem, dagmg_partroword
PUBLIC :: dagmg_setupL1, dagmg_smoothsetup
PUBLIC :: dagmg_applyprec, dagmg_GCR, dagmg_FlexCG
CONTAINS
  SUBROUTINE dagmg_relmem(ijb)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER :: l, ier, ijb
  ...
  SUBROUTINE dagmg_applyprec( N,f,X,ijb,a,ja,ia)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER :: N, ijb
    ...
    IF (iter.GT.1) THEN
       CALL dagmg_prec_matv(1,f,X,S(N+1),S(2*N+1),.TRUE.)
       X(1:N)=tau*X(1:N)
       DO i=2,iter-1
          CALL dagmg_prec_matv(1,f,S,S(N+1),S(2*N+1),.TRUE.)
          X(1:N)=X(1:N)+tau*S(1:N)
          f(1:N)=f(1:N)-tau*S(N+1:2*N)
       END DO
       CALL dagmg_prec_matv(1,f,S,S(N+1),S(2*N+1),.FALSE.)
       X(1:N)=X(1:N)+tau*S(1:N)
    ELSE
       CALL dagmg_prec_matv(1,f,X,S,S(N+1),.FALSE.)
    END IF
    kstat(2,1)=kstat(2,1)+iter
    DEALLOCATE(S)
    RETURN
  END SUBROUTINE dagmg_applyprec
  SUBROUTINE dagmg_partroword(n, a, ja, ia, idiag, w, iw, iext, nzext)
    IMPLICIT NONE
  ...
  SUBROUTINE dagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    ...
    IF (init) THEN
       IF (intmv) THEN
          ...
          END IF
          CALL dagmg_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
          CALL dagmg_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iu,x,1,f,-2,flop)
       ELSE
          IF (intmv2) THEN
             CALL dagmg_matv(N,x,SD,dt(1)%a,dt(1)%ja,dt(1)%ia,transint )
          ELSE
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ITER = ITER + 1
       IF (ijb.GT.201) THEN
          CALL dagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv)
       ELSE
          CALL dagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
       END IF
       IF ( ITER.GT.1 ) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL dagmg_matv(N,S(1+2*N),SD,dt(1)%a,dt(1)%ja,dt(1)%ia,trans )
          ELSE
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE dagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    REAL(kind(0.0d0)) ::  f(N), X(N)
    ...
    IF (init) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL dagmg_matv(N,x,Sc,dt(1)%a,dt(1)%ja,dt(1)%ia,trans )
          ELSE
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID2.GT.TOL2BNORM2 )
       ...
       END IF
       IF (ijb.GT.201) THEN
          CALL dagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv)
       ELSE
          CALL dagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
       END IF
       IF (.NOT.intmv) THEN
          IF (intmv2) THEN
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  RECURSIVE SUBROUTINE dagmg_prec_matv(l,B,X,AX,R,matv,a,ja,ia)
    ...
    IF (nnext.GT.0) THEN
       ...
       END IF
       IF (l+1.EQ.nlev) THEN
          CALL dagmg_DIRseq(nnext,R(BN),2)
...
200 CONTINUE
  ...
  SUBROUTINE dagmg_setupL1(n,a,ja,ia,listrank,ifl)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
   ...
   !              WRITE(iout,996) IRANK,cputmp
    ...
    IF (MOD(ijb,10).GE.3) THEN
       ...
       END IF
       CALL dagmg_mestime(-2,0.0d0,0.0d0)
       CALL dagmg_applyprec(n,f,x,MOD(ijb,10))
    ...
    ELSE IF (nrst.GT.1) THEN
       CALL dagmg_GCR(n,f,x,itef,resid,a,ja,ia,ijob)
    ELSE
       CALL dagmg_FlexCG(n,f,x,itef,resid,a,ja,ia,ijob)
    END IF
    iter=itef
