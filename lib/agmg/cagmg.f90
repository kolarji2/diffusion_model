...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This file provides cagmg and dependencies; cagmg is a
! sequential implementation for complex matrices in single precision
...
! for more details.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!! SUBROUTINE cagmg (MAIN DRIVER): see bottom of this file
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
  ...
  END MODULE cagmg_mem
!!!!!!!!!!!!!!!!!!! END of Internal variables declaration
!!!!!!!!!!!!!!!!!!! TIMING
  SUBROUTINE cagmg_mestime(id,cputm,eltm)
    IMPLICIT NONE
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_mestime
!!!!!!!!!!!!!!!!!!! END of TIMING
  MODULE cagmg_PARDISSO
    TYPE PARDISO_HANDLE
       INTEGER(KIND=8) DUMMY
    END TYPE PARDISO_HANDLE
  END MODULE cagmg_PARDISSO
  MODULE cagmg_PARDISO
    USE cagmg_PARDISSO
    INTERFACE
  ...
  END MODULE cagmg_PARDISO
    SUBROUTINE PARDISO( PT, MAXFCT, MNUM, MTYPE, PHASE, N, A, IA, JA    &
      ...
      STOP
    END SUBROUTINE PARDISO
MODULE cagmg_ALLROUTINES
PRIVATE
PUBLIC :: cagmg_relmem, cagmg_partroword
PUBLIC :: cagmg_setupL1, cagmg_smoothsetup
PUBLIC :: cagmg_applyprec, cagmg_GCR, cagmg_FlexCG
PUBLIC :: cagmg_diagscal
CONTAINS
  SUBROUTINE cagmg_relmem(ijb)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l, ier, ijb
    ...
    IF (.NOT.allzero)                              &
            CALL cagmg_DIRseq(nn(nlev),fdum,-2)
    RETURN
  END SUBROUTINE cagmg_relmem
  SUBROUTINE cagmg_applyprec( N,f,X,ijb,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: N, ijb
    ...
    IF (iter.GT.1) THEN
       CALL cagmg_prec_matv(1,S,X,S(2*N+1),S(3*N+1),.TRUE.,a,ja,ia)
       X(1:N)=tau*X(1:N)
       DO i=2,iter-1
          CALL cagmg_prec_matv(1,S,S(N+1),S(2*N+1),S(3*N+1),.TRUE.,a,ja,ia)
          X(1:N)=X(1:N)+tau*S(N+1:2*N)
          S(1:N)=S(1:N)-tau*S(2*N+1:3*N)
       END DO
       CALL cagmg_prec_matv(1,S,S(N+1),S(2*N+1),S(3*N+1),.FALSE.,a,ja,ia)
       X(1:N)=X(1:N)+tau*S(N+1:2*N)
    ELSE
       CALL cagmg_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.,a,ja,ia)
    END IF
    X(1:N)=X(1:n)*scald(1:N)
    kstat(2,1)=kstat(2,1)+iter
    DEALLOCATE(S)
    RETURN
  END SUBROUTINE cagmg_applyprec
  SUBROUTINE cagmg_partroword(n, a, ja, ia, idiag, w, iw, iext, nzext)
    IMPLICIT NONE
    ...
    ENDDO
    RETURN
  END SUBROUTINE cagmg_partroword
  SUBROUTINE cagmg_diagscal(n,a,ja,ia,idiag,as)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: n, ja(*), ia(n+1), idiag(n)
    ...
    END DO
    RETURN
  END SUBROUTINE cagmg_diagscal
  SUBROUTINE cagmg_csrdlu(n,a,ja,ia,idiag,ao,jao,iup,iext,iextop)
    IMPLICIT NONE
    ...
    iei=iui+nu
       ...
       il(n+1)=ili
    RETURN
  END SUBROUTINE cagmg_csrdlu
  SUBROUTINE cagmg_csrdluT(n,a,ja,ia,idiag,ao,jao,il,iu)
    IMPLICIT NONE
    ...
    iei=iui+nu
       ...
       iu(1)=iui
    RETURN
  END SUBROUTINE cagmg_csrdluT
  SUBROUTINE cagmg_csrmv(n, a, ja, ifja, ia, x, ifx, y, iad, flop, lstin)
    IMPLICIT NONE
    ...
    RETURN
  END SUBROUTINE cagmg_csrmv
  SUBROUTINE cagmg_mcsrmv(n, a, ja, ifja, ia, x, ifx, y, iad, flop)
    IMPLICIT NONE
    INTEGER :: n, ifja, ja(ifja:*), ia(n+1), iad, ifx, i, k, j
    COMPLEX(kind(0.0e0)) :: a(*), x(ifx:*), y(n), t
    REAL(kind(0.0e0)) :: flop
    IF (n.LE.0) RETURN
       ...
       END IF
    RETURN
  END SUBROUTINE cagmg_mcsrmv
  SUBROUTINE cagmg_csrlsolve(n, a, ja, ifja, ia, p, x, y, iunit, flop)
    IMPLICIT NONE
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_csrlsolve
  SUBROUTINE cagmg_csrusolve(n, a, ja, ifja, ia, p, x, y, iunit, flop)
    IMPLICIT NONE
    INTEGER :: n, ifja, ja(ifja:*), ia(n+1), iunit, i, k
    COMPLEX(kind(0.0e0)) :: a(*), p(n), x(n), y(n), t
    REAL(kind(0.0e0)) :: flop
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_csrusolve
  SUBROUTINE cagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    ...
    IF (init) THEN
       ...
       IF (intmv) THEN
          ...
          END IF
          CALL cagmg_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
          CALL cagmg_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iu,x,1,f,-2,flop)
       ELSE
          IF (intmv2) THEN
             CALL cagmg_matv(N,x,SD,dt(1)%a,dt(1)%ja,dt(1)%ia,transint )
          ELSE
             CALL cagmg_matv(N,x,SD,a,ja,ia,trans )
          END IF
          f(1:n)=f(1:n)-SD(1:N)
       END IF
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ...
       IF (.NOT.intmv2) THEN
          ...
          CALL cagmg_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
          S(1+3*N:4*N)=S(1+3*N:4*N)*scald(1:N)
          flop=flop+real(2*N)
       ELSE IF (ijb.GT.201) THEN
          CALL cagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv)
       ELSE
          CALL cagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
       END IF
       IF ( ITER.GT.1 ) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL cagmg_matv(N,S(1+2*N),SD,dt(1)%a,dt(1)%ja,dt(1)%ia,trans )
          ELSE
             CALL cagmg_matv(N,S(1+2*N),SD,a,ja,ia,trans )
          END IF
       END IF
...
999 FORMAT('****     set x equal to the zero vector and exit')
  END SUBROUTINE cagmg_FlexCG
  SUBROUTINE cagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    COMPLEX(kind(0.0e0)) ::  f(N), X(N)
    ...
    IF (init) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL cagmg_matv(N,x,Sc,dt(1)%a,dt(1)%ja,dt(1)%ia,trans )
          ELSE
             CALL cagmg_matv(N,x,Sc,a,ja,ia,trans )
          END IF
          f(1:n)=f(1:n)-Sc(1:N)
       END IF
       dum(2) = SCNRM2(N, f, IONE)**2
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID2.GT.TOL2BNORM2 )
       ...
       IF (.NOT.intmv2) THEN
          fsc(1:N)=f(1:N)*scald(1:N)
          CALL cagmg_prec_matv(1,fsc,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
          Su(1+itm*N:(1+itm)*N)=Su(1+itm*N:(1+itm)*N)*scald(1:N)
          flop=flop+real(2*N)
       ELSE IF (ijb.GT.201) THEN
          CALL cagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv)
       ELSE
          CALL cagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
       END IF
       IF (.NOT.intmv) THEN
          IF (intmv2) THEN
             CALL cagmg_matv(N, Su(1+itm*N), Sc(1+itm*N)             &
...
998 FORMAT('**** The norm of the right hand side is zero:')
999 FORMAT('****     set x equal to the zero vector and exit')
  END SUBROUTINE cagmg_GCR
  SUBROUTINE cagmg_matv(n, x, y, a, ja, ia, transpose,         &
       iext, lstin, l)
    USE cagmg_mem
    IMPLICIT NONE
    ...
    flop = flop + real(2 *(ia(n+1)-ia(1))-n)
    RETURN
  END SUBROUTINE cagmg_matv
  RECURSIVE SUBROUTINE cagmg_prec_matv(l,B,X,AX,R,matv,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l
    ...
    IF (l.EQ.nlev) THEN
       X(1:N)=B(1:N)
       IF (PRESENT(a)) THEN
          CALL cagmg_DIRseq(n,X,2,a,ja,ia)
       ELSE
          CALL cagmg_DIRseq(n,X,2)
       END IF
       RETURN
    END IF
    ...
    ELSEIF (npresmooth.EQ.1) THEN
       CALL cagmg_fwGS(l,B,X,AX,.FALSE.,R)
    ELSE
       update=.FALSE.
       DO is=2,npresmooth,2
          CALL cagmg_fwbwsolve1(l,B,X,AX,update,R)
          update=.TRUE.
       END DO
       IF (MOD(npresmooth,2).EQ.1) THEN
          CALL cagmg_fwGS(l,B,X,AX,.TRUE.,R)
       END IF
    END IF
    IF (nnext.GT.0) THEN
       XN=1
       ...
       IF (npresmooth.EQ.0) THEN
         CALL cagmg_restag(N,nnext,B,R(BN),dt(l)%ind,flop)
       ELSE
         CALL cagmg_restag(N,nnext,AX,R(BN),dt(l)%ind,flop)
       END IF
       IF (l+1.EQ.nlev) THEN
          CALL cagmg_DIRseq(nnext,R(BN),2)
       ELSE IF ( innermax(l+1).LE.1 ) THEN
          AXN=BN+nnext
          RN=AXN+nnext
          CALL cagmg_prec_matv(l+1,R(BN),R(XN),R(AXN),R(RN),.FALSE.)
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
       ELSE
          CALL cagmg_inner_iter(nnext,R(XN),R(BN),l+1)
       END IF
       CALL cagmg_prolag(N,nnext,X,R(XN),dt(l)%ind,flop)
    END IF
    IF (npostsmooth.EQ.0) THEN
    ELSE IF (npostsmooth.EQ.1) THEN
       CALL cagmg_bwGS(l,B,X,AX,matv)
    ELSE
       IF (MOD(npostsmooth,2).EQ.1) THEN
          CALL cagmg_bwGS(l,B,X,AX,.FALSE.)
       END IF
       update=.FALSE.
       DO is=2,npostsmooth,2
          IF (is.GE.npostsmooth-1) update=matv
          CALL cagmg_fwbwsolve2(l,B,X,AX,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE cagmg_prec_matv
  SUBROUTINE cagmg_fwGS(l,B,X,AX,update,R)
    USE cagmg_mem
    IMPLICIT NONE
    ...
    IF (update) THEN
       IF (smoothtp.EQ.1) THEN
         CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a   &
                              ,R,AX,1,flop)
       ELSE
         CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%p   &
                              ,R,AX,1,flop)
       END IF
       X(1:n)=X(1:n)+R(1:n)
       flop=flop+real(n)
       IF (smoothtp.EQ.1) THEN
         CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop)
       ELSE
         CALL  cagmg_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop)
       ENDIF
    ELSE
       IF (smoothtp.EQ.1) THEN
         CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,  &
       ...
       END IF
       IF (smoothtp.EQ.1) THEN
         CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,-1,flop)
       ELSE
         CALL  cagmg_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,-1,flop)
       ENDIF
    END IF
    RETURN
  END SUBROUTINE cagmg_fwGS
  SUBROUTINE cagmg_bwGS(l,B,X,AX,matv)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    ...
    AX(1:n)=B(1:n)
    IF (smoothtp.EQ.1) THEN
      CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,-2,flop)
    ELSE
      CALL  cagmg_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,-2,flop)
    END IF
    IF (smoothtp.EQ.1) THEN
      CALL cagmg_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%a,X,AX,1,flop)
    ELSE
      CALL cagmg_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%p,X,AX,1,flop)
    END IF
    IF (.NOT.matv) RETURN
    IF (smoothtp.EQ.1) THEN
      CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,2,flop)
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_bwGS
  SUBROUTINE cagmg_fwbwsolve1(l,B,X,AX,update,R)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    COMPLEX(kind(0.0e0)) ::  B(*), X(*), AX(*), R(*)
    ...
    IF (.NOT.update) THEN
       ...
       ENDIF
       IF (smoothtp.EQ.1) THEN
         CALL cagmg_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%a    &
                              ,X,R,1,flop)
       ELSE
         CALL cagmg_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%p    &
                              ,X,R,1,flop)
       ENDIF
       AX(1:n)=B(1:n)-R(1:n)
       flop=flop+real(n)
    ...
    ELSE
       ...
       flop=flop+real(2*n)
       IF (smoothtp.EQ.1) THEN
         CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,R(n+1),1,AX  &
                           ,-2,flop)
       ELSE
         CALL  cagmg_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,R(n+1),1,AX  &
                           ,-2,flop)
       ENDIF
    END IF
    RETURN
  END SUBROUTINE cagmg_fwbwsolve1
  SUBROUTINE cagmg_fwbwsolve2(l,B,X,AX,matv,R)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    COMPLEX(kind(0.0e0)) ::  B(*), X(*), AX(*), R(*)
    ...
    n=nn(l)
    IF (smoothtp.EQ.1) THEN
      CALL  cagmg_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,0,flop)
    ELSE
      CALL  cagmg_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,0,flop)
    ENDIF
    R(1:n)=B(1:n)-AX(1:n)
    IF (smoothtp.EQ.1) THEN
      CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a    &
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_fwbwsolve2
  RECURSIVE SUBROUTINE cagmg_inner_iter(n,X,R,l)
    USE cagmg_mem
    IMPLICIT NONE
    ...
    END IF
    ITER = 1
    CALL cagmg_prec_matv(l,R,X,R(1,2),R(1,3),.TRUE.)
    ...
    END IF
    ITER = 2
    CALL cagmg_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
...
200 CONTINUE
    ...
    flop=flop+real(23*N)
    RETURN
  END SUBROUTINE cagmg_inner_iter
  SUBROUTINE cagmg_prolag(n, nc, V, B, ind, flop)
    IMPLICIT NONE
    ...
    flop = flop + real(n)
    RETURN
  END SUBROUTINE cagmg_prolag
  SUBROUTINE cagmg_restag(n, nc, V, B, ind, flop)
    IMPLICIT NONE
    INTEGER :: n, nc, ind(n), k, i
    COMPLEX(kind(0.0e0)) :: V(n), B(nc)
    REAL(kind(0.0e0)) :: flop
    ...
    flop = flop + real(n)
    RETURN
  END SUBROUTINE cagmg_restag
  SUBROUTINE cagmg_setupL1(n,a,ja,ia,listrank,ifl)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    nglp=ngl
    IF (1 /= nlev) THEN
       CALL cagmg_aggregation(1,n,a,ja,ia,nc)
       CALL cagmg_setup(2,nc)
    ELSE
       DEALLOCATE(dt(1)%idiag)
       CALL cagmg_DIRseq(n,fff,1,a,ja,ia)
       nwrkcum=1
    ...
    END IF
       ...
       IF (transint) THEN
          ...
          CALL cagmg_csrdluT(n,a,ja,ia,dt(1)%idiag              &
               ,ap,jap,dt(1)%il,dt(1)%iu)
          DEALLOCATE(dt(1)%idiag)
       ELSE
          ALLOCATE(ap(nz),jap(nz-n),dt(1)%iu(n+1))
          dt(1)%il => dt(1)%idiag
          CALL cagmg_csrdlu(n,a,ja,ia,dt(1)%idiag              &
               ,ap,jap,dt(1)%iu )
          NULLIFY(dt(1)%idiag)
       END IF
       dt(1)%a  => ap
...
958 FORMAT('****','              Weighted complexity:',f9.2)
  END SUBROUTINE cagmg_setupL1
  RECURSIVE SUBROUTINE cagmg_setup(l,n,listrank)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l,n
    ...
    END IF
    IF (l /= nlev) THEN
       CALL cagmg_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc)
       CALL cagmg_setup(l+1,nc)
    ELSE
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(real(ngl(1))/maxcoarset)
          DEALLOCATE(dt(l)%idiag)
          CALL cagmg_DIRseq(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
    ...
    IF (transint) THEN
       ...
       CALL cagmg_csrdluT(n,dt(l)%a,dt(l)%ja,dt(l)%ia,dt(l)%idiag &
    ...
    ELSE
       ...
       dt(l)%iu => dt(l)%ia
       dt(l)%il => dt(l)%idiag
       CALL cagmg_csrdlu(n,dt(l)%a,dt(l)%ja,dt(l)%ia,dt(l)%idiag &
            ,ap,jap )
       DEALLOCATE(dt(l)%a,dt(l)%ja)
...
924 FORMAT('****','        Number of variables:',i12)
  END SUBROUTINE cagmg_setup
  SUBROUTINE cagmg_smoothsetup
    USE cagmg_mem
    IMPLICIT NONE
    ...
    ELSEIF (smoothtp.EQ.2) THEN
       DO l=1,llev
          IF (nn(l).GT.0) THEN
            ALLOCATE(dt(l)%p(nn(l)))
            CALL cagmg_ilu0(nn(l),dt(l)%a,dt(l)%ja,dt(l)%iu,dt(l)%il,dt(l)%p)
          END IF
       END DO
    END IF
    RETURN
  END SUBROUTINE cagmg_smoothsetup
  SUBROUTINE cagmg_ilu0(n,a,ja,iu,il,p)
    IMPLICIT NONE
    ...
    ENDDO
    RETURN
  END SUBROUTINE cagmg_ilu0
  SUBROUTINE cagmg_omegaest(n,a,ja,ia,idiag,si,w,iext)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: n,ja(*),ia(n+1),idiag(n)
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_omegaest
  SUBROUTINE cagmg_aggregation(l,n,a,ja,ia,nc,listrank)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l,n,nc
    INTEGER :: ja(*),ia(n+1)
    ...
    IF (l.EQ.1) THEN
       ...
       CALL cagmg_setCMK(n,ja,ia,dt(l)%idiag,riperm,iperm)
    ELSE
       ALLOCATE(si1(n),ind2(n),iperm(n))
       iperm(1:n)=1
    END IF
    call cagmg_prepareagg(n,a,ja,ia,dt(l)%idiag,ind2,iperm,si1,ndd,l )
    IF (ndd.EQ.n) THEN
       nc=0
       nzc=0
    ...
    IF (smoothtype.EQ.0 .AND. l.EQ.1 .AND. (.NOT.spd)) THEN
        ALLOCATE(w(2*n))
        CALL cagmg_omegaest(n,a,ja,ia,dt(l)%idiag,si1,w )
        DEALLOCATE(w)
    END IF
    ...
    IF (l.GT.1) THEN
       IF (spd) THEN
          CALL cagmg_findpairs_SI(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm )
       ELSE
          CALL cagmg_findpairs_GI(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm )
       END IF
       DEALLOCATE(iperm)
    ELSE
       IF (spd) THEN
          CALL cagmg_findpairs_SI1(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm )
       ELSE
          CALL cagmg_findpairs_GI1(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm )
       END IF
...
10  CONTINUE
    ...
    CALL cagmg_setcg(n,a,ja,ia,dt(l)%idiag,si1,ind2,lcg,nc,an,jan,ian      &
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL cagmg_findpairs_SF(l,np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw )
       ELSE
          CALL cagmg_findpairs_GF(l,np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw )
       END IF
       DEALLOCATE(w,iw)
    ...
    DEALLOCATE(sinn)
    ALLOCATE(dt(l)%ind(n))
    CALL cagmg_setind(nc,ndd,ldd,lcg1,2*m1,dt(l)%ind)
    DEALLOCATE(lcg1,ldd)
       dt(l+1)%a    => an
       dt(l+1)%ja   => jan
       dt(l+1)%ia   => ian
...
908 FORMAT('****  Rmk: Setup performed for the transpose of the input matrix')
  END SUBROUTINE cagmg_aggregation
  SUBROUTINE cagmg_setCMK(n,ja,ia,idiag,riperm,iperm,iext)
    USE cagmg_mem
    IMPLICIT NONE
...
15  CONTINUE
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_setCMK
  SUBROUTINE cagmg_prepareagg(n,a,ja,ia,idiag,ind2,iperm,si,ndd,l,iext)
    USE cagmg_mem
    IMPLICIT NONE
...
100 CONTINUE
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_prepareagg
  SUBROUTINE cagmg_findpairs_GF(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
...
25     CONTINUE
    ...
    RETURN
  CONTAINS
  SUBROUTINE cagmg_checktentagg_GF
...
10  CONTINUE
    acc=.TRUE.
    RETURN
  END SUBROUTINE cagmg_checktentagg_GF
  END SUBROUTINE cagmg_findpairs_GF
  SUBROUTINE cagmg_findpairs_SF(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent )
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l,n,ja(*),ia(n+1),idiag(n),ind(n),lcg(2,*),nc
...
25     CONTINUE
    ...
    RETURN
  CONTAINS
  SUBROUTINE cagmg_checktentagg_SF
    INTEGER , PARAMETER :: mm=max(2**(npass+1),8)
    REAL(kind(0.0e0)) :: W(mm,mm), sig(mm), AGe(mm), v(mm)
    REAL(kind(0.0e0)) :: alpha, alp, tmp, beta, beta0, f1, f2
    INTEGER :: j,jj,k,l,m, setdim, l2, k2
...
10  CONTINUE
    acc=.TRUE.
    RETURN
  END SUBROUTINE cagmg_checktentagg_SF
  END SUBROUTINE cagmg_findpairs_SF
  SUBROUTINE cagmg_findpairs_GI(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc )
    USE cagmg_mem
    IMPLICIT NONE
...
901    FORMAT(i3, &
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_findpairs_GI
  SUBROUTINE cagmg_findpairs_SI(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc )
    USE cagmg_mem
    IMPLICIT NONE
...
901    FORMAT(i3, &
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_findpairs_SI
  SUBROUTINE cagmg_findpairs_GI1(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm )
    USE cagmg_mem
    IMPLICIT NONE
...
901    FORMAT(i3, &
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_findpairs_GI1
  SUBROUTINE cagmg_findpairs_SI1(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm )
    USE cagmg_mem
    IMPLICIT NONE
...
901    FORMAT(i3, &
    ...
    ENDIF
    RETURN
  END SUBROUTINE cagmg_findpairs_SI1
  SUBROUTINE cagmg_lcgmix(nc,m,lcg1,lcg,lcgn)
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_lcgmix
  SUBROUTINE cagmg_setind(nc,ndd,ldd,lcg,m,ind)
    ...
    END DO
    RETURN
  END SUBROUTINE cagmg_setind
  SUBROUTINE cagmg_setcg(n,a,ja,ia,idiag,si,ind,lcg           &
    ...
    ENDDO
    RETURN
  END SUBROUTINE cagmg_setcg
  SUBROUTINE cagmg_LAPACK(n,f,ijb,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: n,ijb
    ...
    END IF
    RETURN
  END SUBROUTINE cagmg_LAPACK
  SUBROUTINE cagmg_DIRseq(n,f,ijob,a,ja,ia)
    USE cagmg_mem
    USE cagmg_PARDISO
    IMPLICIT NONE
    INTEGER :: n,ijob
    ...
    INTEGER, OPTIONAL, TARGET :: ja(*)
    REAL(kind(0.0e0)) , SAVE :: iflop
    COMPLEX(kind(0.0e0)), SAVE :: aaa
    TYPE(PARDISO_HANDLE), ALLOCATABLE, SAVE :: PT(:)
    INTEGER, SAVE :: IPARM(64)
    COMPLEX(kind(0.0e0)), ALLOCATABLE, SAVE :: X(:)
...
!   ------------------------
    ...
    COMPLEX(kind(0.0e0)), ALLOCATABLE, TARGET, SAVE :: coak(:)
    INTEGER :: i, j, k, nnzi, idum(1), kzi
    COMPLEX(kind(0.0e0)) :: td, dum(1)
    ...
    ELSE IF (ijob.EQ.2) THEN
       ...
       flop=flop+iflop
       IF (coasing) THEN
          td=CDOTC(N,coak,IONE,f,IONE)
          CALL CAXPY(N,-td,coak,IONE,f,IONE)
       END IF
    END IF
    RETURN
999 FORMAT('Error detected in PARDISO; error code:', i2,' ; Phase:',i2)
  END SUBROUTINE cagmg_DIRseq
  SUBROUTINE cagmg_sortrows(n,a,ja,ia)
    IMPLICIT NONE
    INTEGER :: n, ia(n+1), ja(*), i, k, jj
    COMPLEX(kind(0.0e0)) :: a(*), aa
    LOGICAL exc
    DO i=1,n
       exc=.TRUE.
       DO WHILE(exc)
    ...
    END DO
    RETURN
  END SUBROUTINE cagmg_sortrows
END MODULE cagmg_ALLROUTINES
!!!!!!!!!!!!!!!!!!! MAIN DRIVER
  SUBROUTINE cagmg( n,a,ja,ia,f,x,ijob,iprint,nrest,iter,tol )
    USE cagmg_mem
    USE cagmg_ALLROUTINES
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    INTEGER :: listrank(1),ifirstlistrank,MPI_COMM_AGMG
    wfo=.TRUE.
    iout=iprint
    IF (iprint.LE.0) THEN
       iout=6
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    ELSE
     IF (preprocessed) THEN
       ...
       kstat=0
     END IF
      CALL cagmg_mestime(-1,0.0e0,0.0e0)
      spd=nrest.EQ.1
      ...
      CALL cagmg_partroword(n,a,ja,ia,dt(1)%idiag,w,iw)
      DEALLOCATE(w,iw)
      ALLOCATE(scald(n),ascal(ia(n+1)-ia(1)))
      CALL cagmg_diagscal(n,a,ja,ia,dt(1)%idiag,ascal)
      CALL cagmg_setupL1(n,ascal,ja,ia,listrank,ifirstlistrank)
      DEALLOCATE(ascal)
      CALL cagmg_smoothsetup
      preprocessed=.TRUE.
      CALL cagmg_mestime(1,cputmp,eltmp)
      IF(wfo)THEN
   ...
   !              WRITE(iout,996) IRANK,cputmp
      ...
      END IF
    END IF
    CALL cagmg_mestime(-2,0.0e0,0.0e0)
    resid=tol
    nrst=nrest
    ...
    IF (MOD(ijb,10).GE.3) THEN
       ...
       END IF
       CALL cagmg_mestime(-2,0.0e0,0.0e0)
       CALL cagmg_applyprec(n,f,x,MOD(ijb,10),a,ja,ia)
       CALL cagmg_mestime(2,cputmp,eltmp)
       cputm=cputm+cputmp
       eltm=eltm+eltmp
       solve=.TRUE.
       RETURN
    ELSE IF (nrst.GT.1) THEN
       CALL cagmg_GCR(n,f,x,itef,resid,a,ja,ia,ijob)
    ELSE
       CALL cagmg_FlexCG(n,f,x,itef,resid,a,ja,ia,ijob)
    END IF
    iter=itef
    CALL cagmg_mestime(2,cputm,eltm)
    solve=.FALSE.
    IF (wfo) THEN
