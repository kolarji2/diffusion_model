...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This file provides zagmgpar and dependencies; zagmgpar is a
! parallel (MPI) implementation for complex matrices in double precision
...
!         modified matrix with mean row and column sum enforced to be zero).
    ...
    REAL(kind(0.0d0)), PARAMETER :: kaptg_blocdia=4+2*npass, kaptg_dampJac=6+2*npass
...
!
    ...
    REAL(kind(0.0d0)) :: wcplex(4),fracnz(maxlev),omeg=1.0d0,flop=0.0d0
...
!!!!!!!!!!!!!!!!!!! END of TIMING
  MODULE zagmgpar_PARDISSO
    TYPE PARDISO_HANDLE
       INTEGER(KIND=8) DUMMY
    END TYPE PARDISO_HANDLE
  END MODULE zagmgpar_PARDISSO
  MODULE zagmgpar_PARDISO
    USE zagmgpar_PARDISSO
    INTERFACE
       SUBROUTINE PARDISO( PT, MAXFCT, MNUM, MTYPE, PHASE, N, A, IA, JA  &
         ...
         COMPLEX(kind(0.0d0)), INTENT(INOUT) :: B(*)
         COMPLEX(kind(0.0d0)), INTENT(OUT)   :: X(*)
       END SUBROUTINE PARDISO
    END INTERFACE
  END MODULE zagmgpar_PARDISO
    SUBROUTINE PARDISO( PT, MAXFCT, MNUM, MTYPE, PHASE, N, A, IA, JA    &
         , PERM, NRHS, IPARM, MSGLVL, B, X, ERROR )
      USE zagmgpar_PARDISSO
      TYPE(PARDISO_HANDLE), INTENT(INOUT) :: PT(*)
...
PRIVATE
PUBLIC :: zagmgpar_relmem, zagmgpar_partroword
PUBLIC :: zagmgpar_setupL1, zagmgpar_smoothsetup
PUBLIC :: zagmgpar_applyprec, zagmgpar_GCR, zagmgpar_FlexCG
PUBLIC :: zagmgpar_setsendrecL1
PUBLIC :: zagmgpar_restorelstrank
PUBLIC :: zagmgpar_diagscal
CONTAINS
  SUBROUTINE zagmgpar_relmem(ijb)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l, ier, ijb
    COMPLEX(kind(0.0d0)) :: fdum(1)
    DEALLOCATE(scald)
    DO l=1,nlev-1
       IF(nn(l).GT.0) THEN
            IF (nn(l+1).GT.0)     &
    ...
    nneigho=0
    IF (.NOT.allzero)                              &
            CALL zagmgpar_MUMPSpar(nn(nlev),fdum,-2)
    RETURN
  END SUBROUTINE zagmgpar_relmem
  SUBROUTINE zagmgpar_applyprec( N,f,X,ijb,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: N, ijb
    ...
    IF (iter.GT.1) THEN
       CALL zagmgpar_prec_matv(1,S,X,S(2*N+1),S(3*N+1),.TRUE.,a,ja,ia)
       X(1:N)=tau*X(1:N)
       DO i=2,iter-1
          CALL zagmgpar_prec_matv(1,S,S(N+1),S(2*N+1),S(3*N+1),.TRUE.,a,ja,ia)
          X(1:N)=X(1:N)+tau*S(N+1:2*N)
          S(1:N)=S(1:N)-tau*S(2*N+1:3*N)
       END DO
       CALL zagmgpar_prec_matv(1,S,S(N+1),S(2*N+1),S(3*N+1),.FALSE.,a,ja,ia)
       X(1:N)=X(1:N)+tau*S(N+1:2*N)
    ELSE
       CALL zagmgpar_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.,a,ja,ia)
    END IF
  ...
  SUBROUTINE zagmgpar_partroword(n, a, ja, ia, idiag, w, iw, iext, nzext)
    IMPLICIT NONE
  ...
  SUBROUTINE zagmgpar_diagscal(n,a,ja,ia,idiag,as)
    ...
    END DO
    CALL zagmgpar_sendrec(scald,idum,-1,0)
    DO i=1,n
  ...
  SUBROUTINE zagmgpar_csrdlu(n,a,ja,ia,idiag,ao,jao,iup,iext,iextop)
    IMPLICIT NONE
  ...
  SUBROUTINE zagmgpar_csrdluT(n,a,ja,ia,idiag,ao,jao,il,iu)
    IMPLICIT NONE
    ...
    iei=iui+nu
       ...
       iu(1)=iui
    RETURN
  END SUBROUTINE zagmgpar_csrdluT
  ...
  SUBROUTINE zagmgpar_mcsrmv(n, a, ja, ifja, ia, x, ifx, y, iad, flop)
    IMPLICIT NONE
    INTEGER :: n, ifja, ja(ifja:*), ia(n+1), iad, ifx, i, k, j
    COMPLEX(kind(0.0d0)) :: a(*), x(ifx:*), y(n), t
    REAL(kind(0.0d0)) :: flop
    IF (n.LE.0) RETURN
       ...
       END IF
    RETURN
  END SUBROUTINE zagmgpar_mcsrmv
  ...
  SUBROUTINE zagmgpar_sendrec(vect,ivect,ishift,l)
    USE zagmgpar_mem
    IMPLICIT NONE
  ...
  SUBROUTINE zagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    ...
    COMPLEX(kind(0.0d0)) :: ALPHA, BET0, RHO, dum(3), td
    ...
    IF (init) THEN
       ...
       END IF
       IF (intmv) THEN
          CALL zagmgpar_sendrec(x,idum,-1,1)
          IF (smoothtp.EQ.1) THEN
             f(1:n)=f(1:n)-x(1:n)/dt(1)%a(1:n)
          ELSE
             f(1:n)=f(1:n)-x(1:n)*(dt(1)%a(1:n)-1/(dt(1)%p(1:n)))
          END IF
          CALL zagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
          CALL zagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iu,x,1,f,-2,flop)
          IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ierr)
          CALL  zagmgpar_csrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%iext,buffi,n+1,f &
               ,-2,flop,dt(1)%lstin)
       ELSE
          IF (intmv2) THEN
             CALL zagmgpar_matv(N,x,SD,dt(1)%a,dt(1)%ja,dt(1)%ia,transint &
             ,dt(1)%iext,dt(1)%lstin,1)
          ELSE
             CALL zagmgpar_matv(N,x,SD,a,ja,ia,trans &
             ,iextL1,lstinL1,0)
          END IF
          f(1:n)=f(1:n)-SD(1:N)
       END IF
       dum(2) = DZNRM2(N, f, IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(2:3),2,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ...
       IF (.NOT.intmv2) THEN
          ...
          CALL zagmgpar_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
          S(1+3*N:4*N)=S(1+3*N:4*N)*scald(1:N)
          flop=flop+dble(2*N)
       ELSE IF (ijb.GT.201) THEN
          CALL zagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv)
       ELSE
          CALL zagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),intmv,a,ja,ia)
       END IF
       IF ( ITER.GT.1 ) THEN
          dum(1) = - ZDOTCM(N, SD, IONE, S(1+3*N), IONE)
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
          BET0=dum(1)
          BET1=BET0/RHO
          ...
          flop=flop+dble(4*N)
          IF (intmv) THEN
             CALL ZAXPY(N, BET1, SD, IONE, S(1+4*N), IONE)
             flop=flop+dble(2*N)
          END IF
       ENDIF
       CALL ZCOPY(N, S(1+3*N), IONE, S(1+2*N), IONE)
       IF (intmv)  THEN
          CALL ZCOPY(N, S(1+4*N), IONE, SD, IONE)
       ELSE
          IF (intmv2) THEN
             CALL zagmgpar_matv(N,S(1+2*N),SD,dt(1)%a,dt(1)%ja,dt(1)%ia,trans &
             ,dt(1)%iext,dt(1)%lstin,1)
          ELSE
             CALL zagmgpar_matv(N,S(1+2*N),SD,a,ja,ia,trans &
             ,iextL1,lstinL1,0)
          END IF
       END IF
       dum(1) =  ZDOTCM(N, S(1+2*N), IONE, SD, IONE)
       dum(2) =  ZDOTCM(N,S(1+2*N),IONE,f,IONE)
       flop=flop+dble(4*N)
       IF (ITER.EQ.1) THEN
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
          IF (.NOT.init) THEN
             BNORM=SQRT(dble(dum(3)))
             RESID0=BNORM
          ...
          END IF
       ELSE
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:2),2,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
       END IF
       RHO=dum(1)
       ALPHA=dum(2)/RHO
       ...
       flop=flop+dble(4*N)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum0,2,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
       RESID=dum0
       RESID=SQRT(RESID)
       RESID=RESID/BNORM
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE zagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, ijb
    COMPLEX(kind(0.0d0)) ::  f(N), X(N)
    ...
    IF (init) THEN
       ...
       ELSE
          IF (intmv2) THEN
             CALL zagmgpar_matv(N,x,Sc,dt(1)%a,dt(1)%ja,dt(1)%ia,trans &
             ,dt(1)%iext,dt(1)%lstin,1)
          ELSE
             CALL zagmgpar_matv(N,x,Sc,a,ja,ia,trans &
             ,iextL1,lstinL1,0)
          END IF
          f(1:n)=f(1:n)-Sc(1:N)
       END IF
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID2.GT.TOL2BNORM2 )
       ...
       IF (.NOT.intmv2) THEN
          fsc(1:N)=f(1:N)*scald(1:N)
          CALL zagmgpar_prec_matv(1,fsc,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
          Su(1+itm*N:(1+itm)*N)=Su(1+itm*N:(1+itm)*N)*scald(1:N)
          flop=flop+dble(2*N)
       ELSE IF (ijb.GT.201) THEN
          CALL zagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv)
       ELSE
          CALL zagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,intmv,a,ja,ia)
       END IF
       IF (.NOT.intmv) THEN
          IF (intmv2) THEN
       ...
       IF (itm.GT.0) THEN
          DO i=0,itm-1
             j=MOD(i,2)+4
             dum(1)=ZDOTCM(N,Sc(1+i*N),IONE,Sc(1+itm*N),IONE)
             CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_DOUBLE_COMPLEX,  &
                  MPI_SUM,ICOMM,ierr)
             bet0=dum(1)
       ...
       dum(2)=ZDOTCM(N, Sc(1+itm*N), IONE, f, IONE)
       flop=flop+dble(6*N)
       IF (ITER.EQ.1) THEN
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE zagmgpar_matv(n, x, y, a, ja, ia, transpose,         &
       iext, lstin, l)
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    LOGICAL :: transpose
    CALL zagmgpar_sendrec(x,idum,-1,l)
  ...
  RECURSIVE SUBROUTINE zagmgpar_prec_matv(l,B,X,AX,R,matv,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l
    ...
    IF (n.EQ.0) THEN
       IF (l+1.EQ.nlev) THEN
          IF (.NOT.allzero)                                      &
               CALL zagmgpar_MUMPSpar(nnext,dum,2)
    ...
    IF (l.EQ.nlev) THEN
       X(1:N)=B(1:N)
       IF (PRESENT(a)) THEN
          CALL zagmgpar_MUMPSpar(n,X,2,a,ja,ia)
       ELSE
          CALL zagmgpar_MUMPSpar(n,X,2)
       END IF
       RETURN
    END IF
    ...
    ELSEIF (npresmooth.EQ.1) THEN
       CALL zagmgpar_fwGS(l,B,X,AX,.FALSE.,R)
    ELSE
       update=.FALSE.
       DO is=2,npresmooth,2
          CALL zagmgpar_fwbwsolve1(l,B,X,AX,update,R)
          update=.TRUE.
       END DO
       IF (MOD(npresmooth,2).EQ.1) THEN
          CALL zagmgpar_fwGS(l,B,X,AX,.TRUE.,R)
       END IF
    END IF
    IF (nnext.GT.0) THEN
       XN=1
       ...
       IF (npresmooth.EQ.0) THEN
         CALL zagmgpar_restag(N,nnext,B,R(BN),dt(l)%ind,flop)
       ELSE
         CALL zagmgpar_restag(N,nnext,AX,R(BN),dt(l)%ind,flop)
       END IF
       IF (l+1.EQ.nlev) THEN
          CALL zagmgpar_MUMPSpar(nnext,R(BN),2)
       ELSE IF ( innermax(l+1).LE.1 ) THEN
          AXN=BN+nnext
          RN=AXN+nnext
          CALL zagmgpar_prec_matv(l+1,R(BN),R(XN),R(AXN),R(RN),.FALSE.)
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
       ELSE
          CALL zagmgpar_inner_iter(nnext,R(XN),R(BN),l+1)
       END IF
    ...
    ELSE
       IF (l+1.EQ.nlev) THEN
          IF (.NOT.allzero)                                      &
               CALL zagmgpar_MUMPSpar(nnext,dum,2)
       ELSE IF ( innermax(l+1).LE.1 ) THEN
          CALL zagmgpar_prec_matv(l+1,B,X,AX,R,.FALSE.)
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
       ELSE
          CALL zagmgpar_inner_fool(l+1)
    ...
    ELSE IF (npostsmooth.EQ.1) THEN
       CALL zagmgpar_bwGS(l,B,X,AX,matv)
    ELSE
       IF (MOD(npostsmooth,2).EQ.1) THEN
          CALL zagmgpar_bwGS(l,B,X,AX,.FALSE.)
       END IF
       update=.FALSE.
       DO is=2,npostsmooth,2
          IF (is.GE.npostsmooth-1) update=matv
          CALL zagmgpar_fwbwsolve2(l,B,X,AX,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE zagmgpar_prec_matv
  SUBROUTINE zagmgpar_fwGS(l,B,X,AX,update,R)
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    IF (update) THEN
       IF (smoothtp.EQ.1) THEN
         CALL zagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a   &
                              ,R,AX,1,flop)
       ELSE
         CALL zagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%p   &
                              ,R,AX,1,flop)
       END IF
       CALL zagmgpar_sendrec(R,idum, -1,l)
       X(1:n)=X(1:n)+R(1:n)
       flop=flop+dble(n)
       IF (smoothtp.EQ.1) THEN
         CALL  zagmgpar_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop)
       ELSE
         CALL  zagmgpar_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,R,1,AX,-1,flop)
       ENDIF
    ELSE
       IF (smoothtp.EQ.1) THEN
         CALL zagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,  &
       ...
       ELSE
         CALL  zagmgpar_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,-1,flop)
       ENDIF
    END IF
    IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ier)
  ...
  SUBROUTINE zagmgpar_bwGS(l,B,X,AX,matv)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    ...
    ELSE
      CALL  zagmgpar_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,X,1,AX,-2,flop)
    END IF
    IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ier)
    CALL  zagmgpar_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iext,buffi,n+1,AX,-2,flop &
         ,dt(l)%lstin)
    ...
    ELSE
      CALL zagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%p,X,AX,1,flop)
    END IF
    IF (.NOT.matv) RETURN
    CALL zagmgpar_sendrec(X,idum,-1,l)
    IF (smoothtp.EQ.1) THEN
  ...
  SUBROUTINE zagmgpar_fwbwsolve1(l,B,X,AX,update,R)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    COMPLEX(kind(0.0d0)) ::  B(*), X(*), AX(*), R(*)
    ...
    IF (.NOT.update) THEN
       ...
       ENDIF
       IF (smoothtp.EQ.1) THEN
         CALL zagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%a    &
                              ,X,R,1,flop)
       ELSE
         CALL zagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,dt(l)%p    &
                              ,X,R,1,flop)
       ENDIF
       CALL zagmgpar_sendrec(X,idum,-1,l)
       AX(1:n)=B(1:n)-R(1:n)
    ...
    ELSE
       ...
       CALL zagmgpar_sendrec(R(n+1),idum,-1,l)
       AX(1:n)=AX(1:n)-R(1:n)
       flop=flop+dble(2*n)
       IF (smoothtp.EQ.1) THEN
         CALL  zagmgpar_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,R(n+1),1,AX  &
                           ,-2,flop)
       ELSE
         CALL  zagmgpar_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,R(n+1),1,AX  &
                           ,-2,flop)
       ENDIF
       IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ier)
       CALL  zagmgpar_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iext,buffi,n+1,AX   &
  ...
  SUBROUTINE zagmgpar_fwbwsolve2(l,B,X,AX,matv,R)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l, n, idum(1), ier
    COMPLEX(kind(0.0d0)) ::  B(*), X(*), AX(*), R(*)
    ...
    ELSE
      CALL  zagmgpar_mcsrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,X,1,AX,0,flop)
    ENDIF
    R(1:n)=B(1:n)-AX(1:n)
    IF (nneighi.GT.0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ier)
  ...
  RECURSIVE SUBROUTINE zagmgpar_inner_iter(n,X,R,l)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER   :: N, ITER, l, ierr, k
    REAL(kind(0.0d0)) ::  RESID, BNORM, det
    COMPLEX(kind(0.0d0)) :: X(N), R(MAX(N,1),*)
    COMPLEX(kind(0.0d0)) :: alpha1,alpha2,bet0,bet1,bet2,rho1,rho2,gamm0,gamm1,zeta
    COMPLEX(kind(0.0d0)) :: dum(5),y1,y2
    COMPLEX(kind(0.0d0)), external :: ZDOTCM
    REAL(kind(0.0d0)), external :: DZNRM2
    INTEGER , parameter :: IONE=1
    ITER = 1
    CALL zagmgpar_prec_matv(l,R,X,R(1,2),R(1,3),.TRUE.)
    dum(1) = ZDOTCM(N,X,IONE,R(1,2),IONE)
    dum(2) = ZDOTCM(N,X,IONE,R,IONE)
    dum(3) = DZNRM2(N, R, IONE)**2
    dum(4) = DZNRM2(N,R(1,2),IONE)**2
    dum(5) = ZDOTCM(N, R, IONE, R(1,2), IONE )
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    ...
    ITER = 2
    CALL zagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = ZDOTCM(N,R(1,3),IONE,R(1,2),IONE)
    dum(2) = ZDOTCM(N,R(1,3),IONE,R,IONE)
    dum(3) = ZDOTCM(N,R(1,3),IONE,R(1,4),IONE)
    IF (.NOT.spd) dum(4) = ZDOTCM(N,X,IONE,R(1,4),IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:4),4,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
    rho2  =dum(3)
...
100 CONTINUE
    ...
    dum(2)=ZDOTCM(N, R(1,2), IONE, R, IONE )
    dum(3) = DZNRM2(N, R, IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    IF (dble(dum(3)).EQ.0.0d0) THEN
...
110 CONTINUE
    ...
    ITER = 2
    CALL zagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = ZDOTCM(N,R(1,4),IONE,R(1,2),IONE)
    dum(2) = ZDOTCM(N,R(1,4),IONE,R,IONE)
...
200 CONTINUE
    dum(1) = DZNRM2(N,R(1,2),IONE)**2
    dum(2) = ZDOTCM(N,R(1,4),IONE,R(1,2),IONE)
    dum(3) = DZNRM2(N,R(1,4),IONE)**2
    dum(4) = ZDOTCM(N,R(1,2),IONE,R,IONE)
    dum(5) = ZDOTCM(N,R(1,4),IONE,R,IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
  ...
  RECURSIVE SUBROUTINE zagmgpar_inner_fool(l)
    ...
    IMPLICIT NONE
    INTEGER :: ITER, l, ierr
    COMPLEX(kind(0.0d0)) :: dum(5)
    REAL(kind(0.0d0)) :: RESID, BNORM, det
    COMPLEX(kind(0.0d0)) :: alpha1,alpha2,bet0,bet1,bet2,rho1,rho2,gamm0,gamm1,zeta
    ITER = 1
    CALL zagmgpar_prec_matv(l)
    dum(1:5) = cmplx(0,0,kind(0.0d0))
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
...
200 CONTINUE
  ...
  SUBROUTINE zagmgpar_setupL1(n,a,ja,ia,listrank,ifl)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    ELSE
       ...
       CALL zagmgpar_MUMPSpar(n,fff,1,a,ja,ia)
       nwrkcum=1
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(dble(ngl(1))/maxcoarset)
      CALL zagmgpar_restorelstrank(ilstinL1,listrank,ifl)
      ifl=n+1
       RETURN
    END IF
       CALL zagmgpar_restorelstrank(ilstinL1,listrank,ifl)
       ...
       IF (transint) THEN
          ...
          CALL zagmgpar_csrdluT(n,a,ja,ia,dt(1)%idiag              &
       ...
       END DO
             ...
             END IF
             IF (wfo) THEN
                WRITE(iout,952)  IRANK,nlctot(1)/dble(n)
                WRITE(iout,953)  IRANK,nlctot(2)/dble(nza)
             END IF
       IF (wff) THEN
...
958 FORMAT('****','              Weighted complexity:',f9.2)
  ...
  RECURSIVE SUBROUTINE zagmgpar_setup(l,n,listrank)
    ...
    IF (wfo) THEN
       ...
       END IF
       IF(wff .AND. ngl(1).GT.0.5d0) THEN
          IF (ngl(2).GT.9.9e10) THEN
             WRITE(prtint(1:12),'(1pe12.5)') ngl(2)
          ELSE
             WRITE(prtint,'(f13.0)') ngl(2)
    ...
    IF (n.EQ.0) THEN
       nc=0
       allzero=.TRUE.
       IF (ngl(1).GT.0.5d0) allzero=.FALSE.
       IF (nlev /= l) THEN
          CALL zagmgpar_setup(l+1,nc)
       ELSE IF (.NOT.allzero) THEN
          CALL zagmgpar_MUMPSpar(n,fff,1)
       END IF
       RETURN
    END IF
    IF (l /= nlev) THEN
    ...
    ELSE
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(dble(ngl(1))/maxcoarset)
          ...
          CALL zagmgpar_MUMPSpar(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
    ...
    IF (transint) THEN
       ...
       CALL zagmgpar_csrdluT(n,dt(l)%a,dt(l)%ja,dt(l)%ia,dt(l)%idiag &
...
935 FORMAT(i3,'*','       Number of local rows:',i12)
  ...
  SUBROUTINE zagmgpar_smoothsetup
    ...
    ELSEIF (smoothtp.EQ.2) THEN
       DO l=1,llev
          IF (nn(l).GT.0) THEN
            ALLOCATE(dt(l)%p(nn(l)))
            CALL zagmgpar_ilu0(nn(l),dt(l)%a,dt(l)%ja,dt(l)%iu,dt(l)%il,dt(l)%p)
          END IF
       END DO
    END IF
    RETURN
  END SUBROUTINE zagmgpar_smoothsetup
  SUBROUTINE zagmgpar_ilu0(n,a,ja,iu,il,p)
    IMPLICIT NONE
    ...
    ENDDO
    RETURN
  END SUBROUTINE zagmgpar_ilu0
  SUBROUTINE zagmgpar_omegaest(n,a,ja,ia,idiag,si,w,iext)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n,ja(*),ia(n+1),idiag(n)
    ...
    ENDIF
    RETURN
  END SUBROUTINE zagmgpar_omegaest
  SUBROUTINE zagmgpar_aggregation(l,n,a,ja,ia,nc,listrank)
    ...
    IF (smoothtype.EQ.0 .AND. l.EQ.1 .AND. (.NOT.spd)) THEN
        ALLOCATE(w(2*n))
        CALL zagmgpar_omegaest(n,a,ja,ia,dt(l)%idiag,si1,w &
            ,dt(l)%iext)
        DEALLOCATE(w)
    END IF
    ...
    IF (l.GT.1) THEN
       IF (spd) THEN
          CALL zagmgpar_findpairs_SI(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       ELSE
          CALL zagmgpar_findpairs_GI(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       END IF
       DEALLOCATE(iperm)
    ELSE
       IF (spd) THEN
          CALL zagmgpar_findpairs_SI1(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       ELSE
          CALL zagmgpar_findpairs_GI1(l,n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       END IF
...
10  CONTINUE
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL zagmgpar_findpairs_SF(l,np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       ELSE
          CALL zagmgpar_findpairs_GF(l,np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       END IF
...
999 CONTINUE
    ...
    isize=MAX(isize,1)
    ALLOCATE (jan(isize))
    CALL zagmgpar_setextnum(nc,l,jan,nzp,listrank)
    IF (nc.EQ.0) THEN
        DEALLOCATE(jan)
        RETURN
    ENDIF
...
100 CONTINUE
  ...
  SUBROUTINE zagmgpar_findpairs_GF(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ...
    npc=0
      ...
      DO WHILE (nmark.LT.nt)
       ...
       DO i = ia(isel),i2
          ...
          sig1=si(isel)-vals
          sig2=si(j)-vals
          IF (sig1.GT.0.0d0) THEN
             del1=rsi
          ELSE
             del1=rsi+2*sig1
          END IF
          IF (sig2.GT.0.0d0) THEN
             del2=rsj
          ELSE
             del2=rsj+2*sig2
          END IF
          IF (vals.GT.0.0d0) THEN
             epsr=repsmach*vals
...
25     CONTINUE
  ...
  SUBROUTINE zagmgpar_checktentagg_GF
    ...
    REAL(kind(0.0d0)) :: alpha, alp, tmp, beta, beta0, f1, f2
    ...
    DO j=1,SetDim
       DO k=1,SetDim
          ...
          END IF
       ENDDO
       IF (sig(j).LT.0.0d0)  AGe(j)=AGe(j)+2*sig(j)
       v(j)=W(j,j)
...
10  CONTINUE
  ...
  SUBROUTINE zagmgpar_findpairs_SF(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent &
    ,iext,iext1)
    USE zagmgpar_mem
    IMPLICIT NONE
...
10  CONTINUE
  ...
  SUBROUTINE zagmgpar_findpairs_GI(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
    USE zagmgpar_mem
...
901    FORMAT(i3, &
  ...
  SUBROUTINE zagmgpar_findpairs_SI(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
    USE zagmgpar_mem
...
901    FORMAT(i3, &
  ...
  SUBROUTINE zagmgpar_findpairs_GI1(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
    USE zagmgpar_mem
...
901    FORMAT(i3, &
  ...
  SUBROUTINE zagmgpar_findpairs_SI1(l,n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
    USE zagmgpar_mem
...
901    FORMAT(i3, &
  ...
  SUBROUTINE zagmgpar_setsendrecL1(n,ja,ia,iext,listrank,ifl,nzext,news)
    USE zagmgpar_mem
    IMPLICIT NONE
...
1010 FORMAT(' The new input matrix has on rank',i9,'has external'       &
  ...
  SUBROUTINE zagmgpar_setextnum(nc,l,iw,imax,listrank)
    USE zagmgpar_mem
    IMPLICIT NONE
       ...
       DO i=1,nneigh
          ...
          IF (kfin.GE.kdeb) THEN
             ...
             DO j=kdeb,kfin
                ...
                END IF
             END DO
             IF (mv) buffo(kfin)=-cmplx(1,0,kind(0.0d0))
             idest=ineigh(i)
             CALL MPI_ISEND(buffo(kdeb),kfin-kdeb+1,MPI_DOUBLE_COMPLEX,&
                  idest,989,icomm,ireqo(ii),ier)
             DO j=ict+1,ict+ic
...
!!#ifdef
  ...
  SUBROUTINE zagmgpar_restorelstrank(ilstin,listrank,ifl)
    USE zagmgpar_mem
    IMPLICIT NONE
  ...
  SUBROUTINE zagmgpar_MUMPSpar(n,f,ijob,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    END IF
    RETURN
  END SUBROUTINE zagmgpar_MUMPSpar
  SUBROUTINE zagmgpar_globnum(n,ja,ia,iext,irg,jcg,l,lstin,nloc)
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    DO i=1,n
      irg(i)=i
    END DO
    CALL zagmgpar_sendrec(dum,irg,ishift,l)
    DO i=1,n
       DO k=ia(i),iext(i)-1
          irg(k)=i+ishift
          jcg(k)=ja(k)+ishift
  ...
  SUBROUTINE zagmgpar_LAPACK(n,f,ijb,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n,ijb
    COMPLEX(kind(0.0d0)), TARGET :: f(n)
    ...
    END IF
    RETURN
  END SUBROUTINE zagmgpar_LAPACK
...
!                 to a STOP statement.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    REAL(kind(0.0d0)), SAVE :: cputm=0.0d0,eltm=0.0d0
    ...
    ijb=MOD(ijob,100)
    itef=iter
    CALL zagmgpar_init(MPI_COMM_AGMG)
    IF (ijob.NE.-1 .AND. &
        ijob.NE. 0 .AND. &
        ijob.NE. 1 .AND. &
        ijob.NE. 2 .AND. &
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    IF (MOD(ijb,10).GE.2) THEN
      IF (ijob.LE.201) THEN
        ...
        CALL zagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1,nzext)
        CALL zagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank,nzext &
                                ,.FALSE.)
        CALL zagmgpar_restorelstrank(ilstinL1,listrank,ifirstlistrank)
        DEALLOCATE(dt(1)%idiag,w,iw)
      END IF
    ELSE
     IF (preprocessed) THEN
       CALL zagmgpar_relmem(-1)
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
       flop=0.0d0
   ...
   !              WRITE(iout,996) IRANK,cputmp
    ...
    IF (MOD(ijb,10).GE.3) THEN
       ...
       END IF
       CALL zagmgpar_mestime(-2,0.0d0,0.0d0)
       CALL zagmgpar_applyprec(n,f,x,MOD(ijb,10),a,ja,ia)
    ...
    ELSE IF (nrst.GT.1) THEN
       CALL zagmgpar_GCR(n,f,x,itef,resid,a,ja,ia,ijob)
    ELSE
       CALL zagmgpar_FlexCG(n,f,x,itef,resid,a,ja,ia,ijob)
    END IF
    iter=itef
